...
public class QueryEngineImpl implements QueryEngine {
	...
	public List<AuthorDetails> getAuthors(Criteria c) throws IllegalArgumentException {
		...
		if ((c.getPublishers() != null && !c.getPublishers().isEmpty())
			...
			if (c.getMinimumPapersPublished() != Integer.MAX_VALUE) {
				authorsByPapers = filterAuthorsByMinimumPaperCount(authorsByPapers, c.getMinimumPapersPublished());
			}
			
			if (c.getConsecutiveYearConstraint() != Integer.MAX_VALUE) {
				authorsByPapers = filterAuthorsByConsecutiveYearConstraint(authorsByPapers, c.getConsecutiveYearConstraint());
			}	
		}
		...
		return getCommonAuthors(authorsByPapers, authorsByName);
	}

	/**
	 * Checks whether the correct parameters are set for the given criteria
	 * as per the query number
	...
	public boolean isValidCriteria(Criteria c) {
		...
		if (queryNum == 1) {
			if (c.getPublishers() == null || c.getPublishers().isEmpty() 
					|| c.getYearSince() == Integer.MAX_VALUE
					|| c.getAuthorName() != null || c.getConsecutiveYearConstraint() != Integer.MAX_VALUE 
	...
	/**
	 * Retrieves authors which are similar to given author.
	 * One author is similar to other, if one has cited other's paper
	 * or vice-versa
	 * @param authorName
	 * @return list
	 */
	...
	/**
	 * Filter out authors which were committee members for given
	 * consecutive years
	 * @param authors
	...
	/**
	 * Filter out authors who have published less than
	 * given number of papers
	 * @param authors
	...
	/**
	 * Parse committee data to find authors and maximum consecutive
	 * years they were part of the same committee
