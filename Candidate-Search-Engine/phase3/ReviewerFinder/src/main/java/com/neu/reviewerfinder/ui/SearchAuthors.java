...
/**
 * This is the class for Searching the authors
 * It returns the list of authors
...
public class SearchAuthors extends Application {
	...
	/**
	 * This method does a Quick Search to get the list of authors based upon
	 * 1) OOPSLA and year since 2010
	 * 2) OOPSLA or ECOOP, at least 2 papers published and not served on committee for two consecutive years
	 * 3) Author name
	 * 4) Keywords in published paper titles
	 * @param group
	 * @param authorName
	 * @param keywordStr
	 ...
	 * @param primaryStage
	 */
	private void quickSearch(ToggleGroup group, String authorName, String keywordStr, ProgressIndicator loadingIndicator,
			Stage primaryStage) {
		...
		Task<Boolean> quickSearch = new Task<Boolean>() {
			{
				setOnSucceeded(workerStateEvent -> {
					if (getValue()) {
						SearchResults ui = new SearchResults();
			...
			protected Boolean call() throws Exception {
				QueryEngine qe = new QueryEngineImpl();
				...
				if (selected == 1) {
					...
					ArrayList<String> keywords = new ArrayList<String>(Arrays.asList(keywordStr.split(",")));
	...
	/**
	 * This method does an Advanced Search to get the list of authors based upon
	 * 1) Journal/conference name
	 * 2) Year published
	 * 3) Years served on committee
	 * 4) Keywords in published paper titles
	 * 5) Minimum papers published
	 * 6) Author name
	 * @param pubStr
	 * @param titleStr
	 * @param dateSince
	 ...
	 * @param primaryStage
	 */
	private void advancedSearch(String pubStr, String titleStr, LocalDate dateSince, String minPaper, Integer consecutiveYear, String authorName, 
		...
		Task<Boolean> advancedSearch = new Task<Boolean>() {
			{
				setOnSucceeded(workerStateEvent -> {
					if (getValue()) {
						...
						SearchError sa = new SearchError();
			...
			protected Boolean call() throws Exception {
				...
				if (pubStr != null && !pubStr.isEmpty()) {
					ArrayList<String> publishers = new ArrayList<String>(Arrays.asList(pubStr.split(",")));
					c.setPublishers(publishers);
				}
				...
				// Minimum Number and Consecutive Year Constraint are just filters
	...
	public void start(Stage primaryStage) {
		...
		RadioButton button2 = new RadioButton("Find authors with atleast two published papers in OOPSLA or ECOOP and did not serve on the committee for two consecutive years");
		...
		group.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
			...
			/**
			 * This method changes the editable property of the text fields in quick search
			 * based upon the query number
			 */
			@Override
		...
		btn.setOnAction(event -> quickSearch(group, authorNameLike.getText(), keywords.getText(), loadingIndicator, primaryStage));
		...
		TextField confText = new TextField();
		confText.setPromptText("Comma Separated Names");
		confText.setId("confText");
		...
		btn1.setOnAction(event -> advancedSearch(confText.getText(), titleText.getText(), datePicker.getValue(), minPaper.getText(), spinner.getValue(), 
				authorName.getText(), loadingIndicator, primaryStage));

		HBox hb1 = new HBox();
		hb1.getChildren().addAll(label, confText);
