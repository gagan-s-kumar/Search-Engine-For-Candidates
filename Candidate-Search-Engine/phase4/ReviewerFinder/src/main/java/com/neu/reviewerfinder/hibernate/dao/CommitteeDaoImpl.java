...
/**
 * @author DJ This is the Hibernate DAO Class for Committee It flushes batches of Committee to the
 *         DB Performs specific functions on the DB for retrieval
 */
public class CommitteeDaoImpl implements CommitteeDao {
  @Override
  public void createAndStoreCommittee(String conference, String name, Integer year, Boolean G_flag,
      Boolean P_flag) {
  ...
  public List<CommitteeConf> getOrderedCommittees() {
    ...
    List<CommitteeConf> result =
        session.createCriteria(CommitteeConf.class).addOrder(Order.asc("name"))
            .addOrder(Order.asc("conference")).addOrder(Order.asc("year")).list();
    session.getTransaction().commit();
    return result;
  }
